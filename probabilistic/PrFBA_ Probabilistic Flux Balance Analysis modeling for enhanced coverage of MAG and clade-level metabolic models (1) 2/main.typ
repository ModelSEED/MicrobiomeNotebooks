...
Microbial communities are ubiquitous @Nayfach2021AMicrobiomes and serve fundamental roles as eukaryotic symbionts @Shoaie2015QuantifyingMicrobiome, @Kumar2019ModellingMicrobiome, @Shoaie2013UnderstandingModeling and pathogens @Mcguigan2015TheLung, @Jenkinson2005OralHealth, industrial assets @Tzamali2011ACommunities, @Pettit2009MixedDiscovery and antagonists @Hillman2022Top-downOperations, and ecological agents of biogeochemical cycling @Griffiths2013InsightsCommunity, @Whitman1998Prokaryotes:Majority, @Dukas2009Deep-SeaConsortia and climate regulation @Cavicchioli2019ScientistsChange, @Orphan2001Methane-consumingAnalysis, @Glass2012TraceOxide. Microbial communities are therefore vital for research fields as diverse as medicine, basic and synthetic biology @Zomorrodi2016SyntheticApplications, materials science, and civil engineering; yet, the principles and the member interactions that manifest in the stable abundances and community behaviors remain unknown @White2015AMicrobiomes, which hinders research in each of these ecosystems @Prosser2007TheEcology. Understanding a microbiome requires knowledge of both which biological functions are occurring within it, to reveal the ecological role of the community, and how those functions are delegated amongst its members, to predict responses of its functions to changing community structure (e.g. a new species replacing an old species) and responses of member interactions to changing environmental conditions.

The functional distribution in a stable microbiome across its members, while difficult to experimentally interrogate, can be elucidated computationally through metabolic modeling @Varma1994StoichiometricW3110. This process, however, requires sequenced genomes of each community member, which is not feasible for particularly natural microbial communities where many members are unculturable. The prominent method to genomically capture microbiome members is through shotgun metagenome sequencing followed by assembly and binning, which yields partial and often contaminated metagenome assembled genomes (MAGs) @Chivian2023. Alternative approaches such as direct annotation and long-read sequencing @Callahan2021 are impeded respectively by short reads and no chromosomal context that create unreliable annotations and low read counts and low abundance members. // citation
MAGs are often sufficient to obtain a reliable phylogenetic placement in genomic space using average nucleotide identity (ANI) approaches like GTDBtk @Chaumeil2022, but they are nearly never complete @Eisenhofer2022 and the proper sequencing depth for characterizing a community remains an open question @Liu2021, which impairs their reliability for characterizing microbiome functions despite their use for this purpose @Baskaran2023. MAGs have several other limitations for microbiome research, including bias towards reconstructing only the most abundant members into even low-quality MAGs, and deep shotgun sequencing of microbiome samples is prohibitively both costly for a single experiment (e.g. large temporal sequences of data or exhaustive spatial variation) and for demanding a lot of extracted DNA. //citation

Amplicon sequencing addresses has a complementary set of strengths and weaknesses as the aforementioned shotgun-sequencing-based approaches. Amplicon sequencing approaches extract a marker gene (typically 16s for microbes) from surrounding DNA and amplify it, typically using PCR approaches. This advantageously captures more species from the same quantity of DNA. // citation
The amplicon sequences can then be clustered into exact sequence variants (ASVs) and annotated using taxonomical classifiers. This method provides no direct information about the functions present in the environment, and phylogenetic classification is typically coarser than ANI-based approaches for even a low-quality MAG, although, function can be inferred based on the functions of reference genomes in the same taxa to which the amplicon sequence has been mapped. // citation
This approach can reveal functional delegation in a micrbiome @Yilmaz2016, with both more uncertainty and more comprehensive strain mapping than MAGs analysis.  // How is it more clear than MAGs given the coarser phylogenetic resolution?

A final approach to understand functional delegation within a complex microbiome circumvents the strain-level limitations by focusing on higher-order clades that can be more confidently supported by MAG- and ASV-based methods. This may be the desirable approach for ecosystems with unique key-stone species whose general niche functions are of interest, instead of examining specific strains in the clade @McDaniel2021, @Fodelianakis2022. A clade-level study of a microbiome, at the investigator's specified taxonomic level, pools the functions of organisms who belong to a common clade or of reference genomes within the same clade to which an ASV was assigned when inferring its function. This approach, however, can create functionally monolithic models, since a single clade can encompass numerous ecological modalities that are difficult to capture while retaining environmental responsiveness for phenotypic expression.
...
Our method begins with the parameterization of a genome or MAG sequence, which is localized within phylogenetic space of a reference database or MAG collection. We used the Genome Taxonomical Database (GTDB) @Parks2022 because it covers vast phylogenetic space and has a convenient tool-kit API @Chaumeil2022 that is integrated into KBase for rapid and reproducible computations @Arkin2018. // The threshold for acquiring the set of is ____
